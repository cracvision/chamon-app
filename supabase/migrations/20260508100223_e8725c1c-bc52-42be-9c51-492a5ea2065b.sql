-- Atomic executor for agent actions.
-- security definer because we bypass RLS to do the SELECT FOR UPDATE + writes
-- in a single transaction; we re-check auth.uid() = user_id manually.
...
BEGIN
  ...
  -- Lock the row; serializes concurrent executions of the same action_id
  SELECT * INTO _action
  ...
  -- Already executed → return prior result, no-op
  ...
  -- Idempotency check: another executed action with same key?
  ...
  IF _action.action_type = 'create_task' THEN
    INSERT INTO public.tasks (
      user_id, mission_id, title, notes, due_date, friction_level, is_today
    ) VALUES (
      _uid,
  ...
  ELSIF _action.action_type = 'create_mission' THEN
    INSERT INTO public.missions (
      user_id, area_id, title, description, priority, due_date, reward_text
    ) VALUES (
      _uid,
  ...
  ELSIF _action.action_type = 'update_task' THEN
    IF NULLIF(_payload->>'task_id','') IS NULL THEN
      RAISE EXCEPTION 'task_id required for update_task' USING ERRCODE = '22023';
    END IF;
...
REVOKE ALL ON FUNCTION public.execute_agent_action(uuid) FROM public;
GRANT EXECUTE ON FUNCTION public.execute_agent_action(uuid) TO authenticated;
